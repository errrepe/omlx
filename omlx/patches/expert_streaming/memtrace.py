...
class MemTracer:
    ...
    def flush(self) -> None:
        """Flush (line-buffered) rows to disk. Safe to call any time."""
        with self._lock:
    ...
    def record(self, event: str, _light: bool = False, **fields: Any) -> None:
        """Append one sample row for ``event`` with arbitrary extra fields.

        The expensive part (``_sample()`` — the MLX allocator plus libproc
        ...
        events (per-layer-call routing traces) where only the fields
        matter and a libproc+MLX probe per event would tax the hot path.
        """
        with self._lock:
            self._seq += 1
            ...
            if "layer" in fields and "proj" in fields:
                ...
                event_seq = self._event_seq[k]
        # Outside the lock: the sampler is the slow part.
        sample = {} if _light else self._sample()
        with self._lock:
            row: dict[str, Any] = {
                "seq": seq,
    ...
    # flush() and close() live next to _open() above.
...
def _build_tracer() -> MemTracer | _NullTracer:
    ...
    if _TRACE_PATH == "1":
        ...
        return tracer
    try:
        tracer = MemTracer(path=_TRACE_PATH, sample_every=_SAMPLE_EVERY)
    except OSError as e:
