...
from .telemetry import read_stats as read_stats  # noqa: F401
...
_BANK_PIPE_ENV = (
    os.environ.get("OMLX_EXPERT_STREAMING_BANK_PIPE", "0") == "1"
)
...
class _ShardReader:
    ...
    def _read_into(self, abs_off: int, out: np.ndarray) -> None:
        ...
        if _PCACHE_PROBE_ENV:
            # Sampled BEFORE the read: preadv is a buffered read, so whatever
            # mincore reports as resident is what this read will be served
            # from, and the remainder is what the device has to deliver.
            #
            # 1-in-N: mincore(2) is a syscall, and probing every read cost
            # 3.6% decode throughput, which is not a price a diagnostic gets
            # to charge. itertools.count is atomic, so concurrent pool
            # workers cannot collide on the same slot.
            tick = next(_PCACHE_SAMPLE_TICK)
            page_cache_probe().seen(n)
            if _PCACHE_PROBE_ENV == 1 or tick % _PCACHE_PROBE_ENV == 0:
                hot = _mincore_resident_bytes(self._mmap, abs_off, n)
                if hot is None:
                    page_cache_probe().failed()
                else:
                    page_cache_probe().record(hot, max(0, n - hot))
        try:
...
class ExpertBackingStore:
    ...
    def __init__(
        ...
        # Runtime-arm registry (weak so stores die freely).
        register_backing(self)
...
each key to its shard filename, so stacked banks spilled outside the
    ...
    def read_expert_into(
        ...
        *,
        ...
        if _BANK_PIPE_ENV:
            return self._read_expert_into_piped(
                components, outs, merge_gap=merge_gap, max_run_bytes=max_run_bytes
            )
        ...
        for (key, eids), out in zip(components, outs):
            ...
            if not self._read_component_into(
                key, eids, out, merge_gap, max_run_bytes, _tel_on
            ):
                return False
        return True
    ...
    def _plan_component_runs(
        self, eids, n, rp, merge_gap, max_run_bytes, stages, tel_on
    ):
    ...
    @staticmethod
    def _scatter_run_rows(eids, out, rp, stages, tel_on, first, js, buf) -> None:
        ...
        _t_sc = time.perf_counter_ns() if tel_on else 0
        for j in js:
    ...
    def _read_component_into(
        self, key, eids, out, merge_gap, max_run_bytes, tel_on
    ) -> bool:
        """One (key, eids) component of ``read_expert_into``.

        Reader resolution, run planning, the windowed reads, and the
        per-component telemetry record. False → the caller fails the
        ...
        if n == 0:
            return True
        resolved = self._resolve_component_reader(
            key, eids, out, n, stages, tel_on
        )
        if resolved is None:
            return _record_failed()
        reader, rp = resolved
        runs = self._plan_component_runs(
            eids, n, rp, merge_gap, max_run_bytes, stages, tel_on
        )
        ...
        if len(runs) == 1:
            ...
            if _RUN_WINDOW_COMPLETION:
                ok = self._read_runs_completion_window(
                    runs, reader, rp, eids, out, stages, run_times,
                    tel_on, _owner_tag, io_exec, ptel,
                )
            else:
                ok = self._read_runs_ordered_window(
                    runs, reader, rp, eids, out, stages, run_times,
                    tel_on, _owner_tag, io_exec, ptel,
                )
            if not ok:
                return _record_failed()
        self._record_component_read(comp_t0, runs, rp, stages, run_times, tel_on)
        return True

    def _read_runs_completion_window(
        self, runs, reader, rp, eids, out, stages, run_times,
        tel_on, owner_tag, io_exec, ptel,
    ) -> bool:
        """Completion-order sliding window (RUN_WINDOW='completion'):
        whichever read finished first is reaped next so a slow run cannot
        head-of-line-block the rest; the window refills one-for-one so
        <= _RUN_IO_QD reads are ever in flight. False → caller fails."""
        ...
        def submit_next():
            ...
            submit_ts = time.perf_counter_ns() if tel_on else 0

            def _timed_run(off, buf, submit_ts, rp_bytes, js, first):
                t0 = time.perf_counter_ns() if tel_on else 0
                try:
                    reader._read_into(off, buf)
                    if _SCATTER_WORKER:
                        _t_sc = (
                            time.perf_counter_ns() if tel_on else 0
                        )
                        for j in js:
                            base = (eids[j] - first) * rp_bytes
                            out[j, :] = buf[base : base + rp_bytes]
                        if tel_on and stages is not None:
                            stages["scatter_us"].append(
                                (time.perf_counter_ns() - _t_sc)
                                // 1000
                            )
                finally:
                    if tel_on and run_times is not None:
            ...
            fn = _timed_run
            if tel_on and ptel is not None:
                fn = ptel.wrap(submit_ts, fn, owner=owner_tag)
                ptel.submit_notice(owner=owner_tag)
            fut = io_exec.submit(
                fn, off, buf, submit_ts, rp.expert_bytes, js, first
            )
        ...
        while window:
            ...
            if tel_on and len(window) == 1:
                last_wait_t0 = _t_ww
            done, _ = wait(
                list(window),
                timeout=_IO_TIMEOUT_S if _IO_TIMEOUT_S > 0 else None,
            ...
            for fut in done:
                ...
                if _await_io_future(fut):
                    if not _SCATTER_WORKER:
                        self._scatter_run_rows(
                            eids, out, rp, stages, tel_on, first, js, buf
                        )
                else:
                    ok = False
                submit_next()
            if not ok:
                for fut in list(window):
                    # Drain so a failed read's fallback accounting
                    # is not lost with the discarded future.
        ...
        for idx, (off, first, count, js) in enumerate(runs):
            ...
            window.append(
                (
                    off, first, js, buf,
                    io_exec.submit(
                        fn, off, buf, submit_ts, rp.expert_bytes,
                        js, first,
                    ),
                )
            )
            if len(window) >= _RUN_IO_QD or idx == len(runs) - 1:
                ...
                if not _wait_future(wfut):
                    ok = False
                if ok and not _SCATTER_WORKER:
                    self._scatter_run_rows(
                        eids, out, rp, stages, tel_on, wfirst, wjs, wbuf
                    )
            if not ok:
    ...
    def _read_expert_into_piped(
        ...
        *,
        ...
        ~3-9 scattered runs serialize into three round trips of queue
        depth. Here every run descriptor carries its own (out, eids, rp)
        so all components' runs share the same _RUN_IO_QD window; the
        device never idles between components. Bytes land in the same
        caller buffers with the same per-descriptor disjoint-row contract,
        so output is identical — only the submission schedule differs.
        """
        _tel_on = self.read_telemetry is not None and self.read_telemetry.enabled
        if len(components) != len(outs):
            return False
        ...
        for (key, eids), out in zip(components, outs):
            if out.dtype != np.uint8 or out.ndim != 2:
                return _record_failed()
            n = len(eids)
            _t_rs = time.perf_counter_ns() if _tel_on else 0
            try:
                readers = [self._reader_for_key(key, int(e)) for e in eids]
            except Exception:
                return _record_failed()
            ...
            for first, count in segment_runs(
                ...
                off = rp.tensor_abs_off + first * rp.expert_bytes
                all_runs.append(
                    (out, eids, rp, reader, off, first, count, order[lo:hi])
                )
                total_bytes += count * rp.expert_bytes
            if _tel_on and stages is not None:
                stages["plan_us"].append((time.perf_counter_ns() - _t_pl) // 1000)
        if not all_runs:
            return True

        def _scatter_desc(out, eids, rp, first, js, buf) -> None:
        ...
        def _submit(desc):
            (out, eids, rp, reader, off, first, count, js) = desc
            ...
            def _timed_run(reader, off, buf, submit_ts, desc):
                (out, eids, rp, _r, _o, first, _c, js) = desc
                t0 = time.perf_counter_ns() if _tel_on else 0
                try:
                    reader._read_into(off, buf)
                    if _SCATTER_WORKER:
                        _scatter_desc(out, eids, rp, first, js, buf)
                finally:
            ...
            return io_exec.submit(fn, reader, off, buf, submit_ts, desc), buf
        ...
        if _RUN_WINDOW_COMPLETION:
            ...
            window: dict = {}
            pending = iter(all_runs)

            def submit_next():
                desc = next(pending, None)
                if desc is None:
                    return None
                fut, buf = _submit(desc)
                window[fut] = (desc, buf)
                return fut
            ...
            while window:
                ...
                if _tel_on and stages is not None:
                    ...
                    if last_wait_t0 is not None:
                        ...
                        last_wait_t0 = None
                for fut in done:
                    desc, buf = window.pop(fut)
                    if _await_io_future(fut):
                        if not _SCATTER_WORKER:
                            _scatter_desc(
                                desc[0], desc[1], desc[2], desc[5],
                                desc[7], buf,
                            )
                    else:
                        ok = False
            ...
            window: list = []
            for idx, desc in enumerate(all_runs):
                fut, buf = _submit(desc)
                window.append((desc, buf, fut))
                if len(window) >= _RUN_IO_QD or idx == len(all_runs) - 1:
                    wdesc, wbuf, wfut = window.pop(0)
                    if not _wait_future(wfut):
                        ok = False
                    if ok and not _SCATTER_WORKER:
                        _scatter_desc(
                            wdesc[0], wdesc[1], wdesc[2], wdesc[5],
                            wdesc[7], wbuf,
                        )
                if not ok:
                    for _d, _b, fut in window:
                        _await_io_future(fut)
                    return _record_failed()
            last_wait_t0 = None
            for wi, (wdesc, wbuf, wfut) in enumerate(window):
        ...
        if _tel_on and stages is not None:
            ...
            self.read_telemetry.record_call(
                runs=len(all_runs),
                bytes_=total_bytes,
                run_sizes=[d[6] for d in all_runs],
                requested_inflight=min(_RUN_IO_QD, len(all_runs)),
                timings=stages,
            )
        return True
    ...
    # ------------------------------------------------------------------ v2
    # Fused expert bank (Cherenkov steal): one bank file per MoE LAYER,
    # every projection of an expert fused into a single record. The demand
    # unit of a layer-call is the expert, so a miss costs ONE preadv
    # (~2.7 MiB on this checkpoint) instead of one command per projection
    # -- command size is throughput on NVMe (fixed per-command latency;
    # 2 MB reads measured at 2x the GB/s of 1 MB).

    # Source keys keep resolving to the ORIGINAL shards (weight_map is
    # untouched): the fused bank serves only the explicit fused read path,
    # so a partial attach can never serve drifted bytes to the per-expert
    # fallback.

    def attach_expert_bank(
        self, bank_dir: str | Path | None = None
    ) -> Tuple[bool, str, int]:
        """Attach the packed v2 fused bank of this model.

        *bank_dir* overrides the default <model>/.omlx/expert_bank
        location — lets a user point the model at an existing bank kept
        elsewhere (a read-only volume, a second SSD, a shared install).

        Returns (ok, reason, n_groups). On success every source expert key
        in the manifest resolves through packed_component() and per-layer
        fused readers are available to read_expert_bank_fused(). Idempotent.
        """
        from .expert_bank_pack import bank_dir_for, bank_status_for

        if getattr(self, "_bank_attached", False):
            return (True, "ok", self._bank_groups)
        ok, why, manifest = bank_status_for(self.model_path, bank_dir)
        if not ok or manifest is None:
            return (False, why, 0)
        n_groups = 0
        comp_map: Dict[str, Tuple[str, int, int]] = {}
        readers: Dict[str, _ShardReader] = {}
        bdir = (
            Path(bank_dir).expanduser()
            if bank_dir is not None
            else bank_dir_for(self.model_path)
        )
        if not bdir.is_absolute():
            bdir = (self.model_path / bdir).resolve()
        else:
            bdir = bdir.resolve()
        try:
            for g in manifest.get("groups") or []:
                fname = str(g.get("file") or "")
                fpath = bdir / fname
                if not fpath.is_file():
                    return (False, f"bank file missing: {fname}", 0)
                fkey = str(fpath)
                with self._readers_lock:
                    if self._closed:
                        return (False, "store closed", 0)
                    reader = readers.get(fkey)
                    if reader is None:
                        reader = self._readers.get(fkey)
                    if reader is None:
                        reader = _ShardReader(fpath)
                        ...
                        if winner is not reader:
                            ...
                            reader = winner
                    readers[fkey] = reader
                packed_key = str(g.get("packed_key") or "")
                if packed_key and packed_key not in reader.header:
                    return (False, f"bank file {fname} lacks {packed_key}", 0)
                for c in g.get("components") or []:
                    comp_map[str(c.get("key"))] = (
                        packed_key,
                        int(c.get("offset") or 0),
                        int(c.get("bytes") or 0),
                    )
                # Register the packed key on the source-resolution path so
                # _rp_for shape/dtype derivation and run coalescing work
                # through the regular reader machinery.
                self._weight_map[packed_key] = fname
                self._key_to_reader[("src", packed_key)] = reader
                # A v3 group written in co-activation order is a stale
                # layout now — the ordering strategy was removed (V3: the
                # read mechanism, not the order, carried the gains).
                # Refuse it loudly rather than serve permuted rows.
                if g.get("row_of"):
                    return (
                        False,
                        f"bank group {g.get('prefix')} was packed in "
                        "co-activation order (stale) — rebuild the bank",
                        0,
                    )
                n_groups += 1
        except Exception as exc:
            return (False, f"attach failed: {exc}", 0)
        if not comp_map:
            return (False, "manifest carries no components", 0)
        self._packed_components = comp_map
        self._bank_prefixes = {
            str(g.get("prefix")): str(g.get("packed_key"))
            for g in manifest.get("groups") or []
        }
        self._bank_readers = readers
        self._bank_groups = n_groups
        self._bank_attached = True
        return (True, "ok", n_groups)

    def packed_component(self, key: str):
        """(packed_key, offset, bytes) of *key* inside its fused record."""
        m = getattr(self, "_packed_components", None)
        if not m:
            return None
        return m.get(key)

    def bank_reader_for_prefix(self, layer_prefix: str):
        """Fused-bank reader of one MoE layer, or None when not attached."""
        pk = getattr(self, "_bank_prefixes", {}).get(layer_prefix)
        if pk is None:
            return None
        return self._key_to_reader.get(("src", pk))

    def read_expert_bank_fused(
        self, layer_prefix: str, expert_ids: list[int]
    ):
        """Read whole-expert fused records for *expert_ids* of one layer.

        Returns (bank, record_bytes) where bank is a raw uint8 array of
        shape (len(ids), record_bytes) ordered like expert_ids, or None
        when the fused bank is not attached for that layer. One coalesced
        preadv per run of consecutive ids -- the Cherenkov storage unit.
        """
        reader = self.bank_reader_for_prefix(layer_prefix)
        if reader is None:
            return None
        pk = self._bank_prefixes[layer_prefix]
        rp = reader._rp_for(pk)
        ids = [int(e) for e in expert_ids]
        bank = np.empty((len(ids), rp.expert_bytes), dtype=np.uint8)
        # merge_gap stays 0: bridging holes reads unrouted experts (dead
        # bytes) — a measured net loss on a saturated disk.
        if not self.read_expert_into(
            [(pk, ids)], [bank],
            merge_gap=0,
        ):
            return None
        return bank, rp.expert_bytes
