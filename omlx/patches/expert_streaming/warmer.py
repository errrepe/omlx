...
class PrefillHotnessRecorder:
    ...
    def maybe_seed(
        ...
        """Fire once, at the first decode-shaped call after a prefill."""
        if (
            self.seeded
            or not self.saw_prefill
            or not _is_decode_call(positions, seq_len)
        ):
            return
        self.seeded = True
    ...
    def _seed_lru(self) -> int:
        ...
        try:
            _note = getattr(self.cache, "note_phase", None)
