...
class ExpertCache:
    ...
    def _install(self, e: int, payload: list | None = None) -> int:
        # Bytes are fetched BEFORE touching slot_of/free/map: a mid-fetch
        ...
        try:
            ...
            # A partial write invalidates the evicted expert too: row bytes
            # are suspect mid-write, so the slot goes back to free while the
            # victim stays evicted (its data is already clobbered). Same
            # rule as the DSv4.1 rollback's interrupted-entry branch.
    ...
    def ensure(self, idx: mx.array) -> None:
        ...
        with self._lock:
            pool = _io_pool()
            queue = (
                [e for e in needed if e not in self.slot_of]
                if pool is not None
                else []
            )
            window = _io_batch()
