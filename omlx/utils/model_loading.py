...
def maybe_apply_pre_load_patches(
    ...
    if model_settings is not None:
        ...
        validate_moe_expert_offload(
            ...
            model_type=_pre_mtype,
        )

    # NOTE: the MoE expert-offload compatibility gate lives at the END of
    # this function, after all model compat patches have run. Gating here
    # would abort model-essential registrations (e.g. qwen4_exp) and brick
    # the load with a misleading error instead of the real reason.
    ...
    # MoE expert-offload compatibility gate — deliberately LAST: every
    # model compat patch above (qwen4_exp registration, sanitize wraps,
    # bonsai qmv) must already have run, so a rejection raises the REAL
    # reason instead of bricking the load with a misleading downstream
    # error. Still before any weight load, honoring the fail-fast contract.
