...
def get_lifetime_max_phys_footprint(pid: int | None = None) -> int:  # noqa: C901
...
# --------------------------------------------------------------------------- #
# Externally-wired byte accounting (Fase M)                                    #
# --------------------------------------------------------------------------- #
# Some memory this process holds is wired OUTSIDE MLX's allocator tracking --
# notably full-bank expert-streaming artifacts, mmap'd and bound via
# ``newBufferWithBytesNoCopy`` (custom_kernels/expert_bank_wrap). Such bytes
# ARE counted by ``mx.get_active_memory()`` but NOT by the phys_footprint
# ledger above. Any consumer comparing an active-memory sample against a
# phys/ceiling budget (admission, throttle, OOM, load-delta sizing)
# over-reports usage by that external total unless it discounts them.
#
# Contract (load-bearing, adapted from jundot/omlx PR #3437 by @alytaphoenix,
# Apache-2.0): DISCOUNT AT THE SAMPLE SITE; caches store discounted values;
# no consumer discounts twice. This module never samples MLX (and never
# imports mlx) -- it is pure arithmetic over a lock-snapshotted provider
# registry, safe from any thread/async context, so each call site keeps its
# own already-correct sampling discipline. Providers are snapshotted under the
# lock but CALLED outside it (a slow provider cannot stall unrelated call
# sites) and must be cheap, non-blocking, and never touch MLX; exceptions are
# swallowed.
