...
class TestLegacyChunkHostScan:
    """The halving scan runs on one host copy — no per-slice mx syncs."""

    def test_chunk_boundaries_preserved(self, tmp_path):
        ...
        wrapped = OffloadSwitchGLU(glu, capacity=2, disk=view)

        x = mx.zeros((6, 64), dtype=mx.float32)
        # 6 tokens x 2 experts, all 4 experts touched -> distinct > cap=2
        # -> chunked path. Boundaries must equal a plain chunked reference.
        indices = mx.array([[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]])
        out = wrapped(x, indices)
        mx.eval(out)
        assert out.shape == (6, 2, 64)  # (tokens, top_k, dims)

        # Same math as one-shot reference at full residency.
        wrapped_full = OffloadSwitchGLU(glu, capacity=n, disk=view)
        ref = wrapped_full(x, indices)
        mx.eval(ref)
        assert mx.allclose(out, ref)
